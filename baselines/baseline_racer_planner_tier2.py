...
def convex_combination(vec1, vec2, coeff):
    # 0 <= coeff <= 1
    # the closer to 1, the closer to vec1
    result_vec = airsim.Vector3r(0,0,0)
    result_vec.x_val = (1-coeff) * vec1.x_val + coeff * vec2.x_val  
    result_vec.y_val = (1-coeff) * vec1.y_val + coeff * vec2.y_val 
    result_vec.z_val = (1-coeff) * vec1.z_val + coeff * vec2.z_val 
    return result_vec
...
class BaselineRacer(object):
    ...
    def odometry_callback(self):
        ...
        if self.last_gate_passed_idx == -1:
            if (self.last_gate_idx_moveOnSpline_was_called_on == -1):
                ...
                self.fly_to_first_point_with_moveOnSpline()
        ...
        if (self.finished_race == False):
            ...
            if dist_from_next_gate < self.gate_passed_thresh[self.next_gate_idx]:
                ...
                if (not(self.last_gate_idx_moveOnSpline_was_called_on == self.next_gate_idx) and not self.finished_race):
                    self.fly_to_next_point_with_moveOnSpline()
            ...
            # if L2_norm(self.curr_lin_vel) < 0.3 and self.last_gate_idx_moveOnSpline_was_called_on != 0:
    ...
    def fly_to_first_point_with_moveOnSpline(self):
        next_gate_position = self.gate_poses_ground_truth[self.next_gate_idx].position
        current_drone_position = self.airsim_client.simGetVehiclePose(vehicle_name=self.drone_name).position
        next_point = convex_combination(next_gate_position, current_drone_position, 0.5)
        
        self.airsim_client.moveOnSplineAsync([next_point], 
                                             vel_max=self.vel_max[self.next_gate_idx],
                                             acc_max=self.acc_max[self.next_gate_idx], 
                                             add_position_constraint=True, 
                                             add_velocity_constraint=False, 
    ...
    def fly_to_next_point_with_moveOnSpline(self):
        next_gate_position = self.gate_poses_ground_truth[self.next_gate_idx].position
        current_drone_position = self.airsim_client.simGetVehiclePose(vehicle_name=self.drone_name).position
        next_point = convex_combination(next_gate_position, current_drone_position, 0.5)
