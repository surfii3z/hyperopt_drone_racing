...
class BaselineRacer(object):
    def __init__(self, drone_name = "drone_1", viz_traj=True, viz_traj_color_rgba=[1.0, 0.0, 0.0, 1.0], viz_image_cv2=True):
        ...
        # self.best_race_time_arr = [5.91, 7.32, 9.52, 11.84, 16.25, 19.71, 34.71, 38.33, 39.84, 41.64, 49.72, 51.33, 54.52, 56.46]
        ...
        # self.best_hyper.v = np.array([25.72, 27.03, 16.9, 22.35, 34.67, 19.5, 19.45, 15.7, 24.16, 21.02, 13.08, 32.88, 26.73, 31.48])
        # self.best_hyper.a = np.array([86.14, 149.89, 121.75, 136.92, 98.74, 54.79, 43.08, 127.37, 96.11, 58.28, 50.0, 136.24, 50.0, 41.0])
        # self.best_hyper.d = np.array([4.46, 6.2, 3.5, 6.42, 3.52, 3.57, 4.84, 6.01, 3.5, 4.51, 3.65, 5.45, 3.5, 2.0])
    ...
    def update_hyper_paramters(self, curr_race_time_arr, best_race_time_arr):
        ...
        if curr_race_time_arr[-1] < best_race_time_arr[-1]: # wins
            ...
            for idx in range(FINISH_GATE_IDX + 1):
                if curr_race_time_arr[idx] < self.best_race_time_arr[idx] and abs(curr_race_time_arr[idx] - self.best_race_time_arr[idx]) > 1.5:
            ...
            for idx in range(FINISH_GATE_IDX + 1):
                if curr_race_time_arr[idx] < self.best_race_time_arr[idx] and abs(self.best_race_time_arr[idx] - curr_race_time_arr[idx]) > 1.5:
                ...
                elif abs(self.best_race_time_arr[idx] - curr_race_time_arr[idx]) <= 1.5:
...
def main(args):
    ...
    # new_hyper.v = np.array([25.72, 27.03, 16.9, 22.35, 34.67, 19.5, 19.45, 15.7, 24.16, 21.02, 13.08, 32.88, 26.73, 31.48])
    # new_hyper.a = np.array([86.14, 149.89, 121.75, 136.92, 98.74, 54.79, 43.08, 127.37, 96.11, 58.28, 50.0, 136.24, 50.0, 41.0])
    # new_hyper.d = np.array([4.46, 6.2, 3.5, 6.42, 3.52, 3.57, 4.84, 6.01, 3.5, 4.51, 3.65, 5.45, 3.5, 2.0])
    # new_hyper.d[-1] = 2
